...
export function Feed() {
  ...
  const [posts, setPosts] = useState<Post[]>(POSTS_DEMO); // Start with demo posts immediately
  const [isLoading, setIsLoading] = useState(false); // No need for initial skeleton if starting with demo
    ...
    const loadPosts = async () => {
      try {
        ...
        const unsubscribe = onSnapshot(
          ...
          (_error) => {
            if (active) console.log('Firebase posts listeners failed, keeping demo posts');
          }
        );
        
        return () => unsubscribe();
      } catch (error) {
        if (active) console.error('Error loading posts:', error);
      }
    };
